...
public class ExcelRead {
	...
	public static List<Map<String, String>> read(ReadOption readOption) {
		...
		/*
		 * 각 Row를 리스트에 담는다.
		 * 하나의 Row 는 하나의 Map으로 표현되며
		 * List 에는 모든 Row 가 포함될 것이다.
		 */
		...
		for(int rowIndex = readOption.getStartRow() - 1; rowIndex < numOfRows; rowIndex++) {
			...
			/*
			 * 워크북에서 가져온 시트에서 rowIndex 에 해당하는 Row 를 가져온다.
			 * 하나의 Row 는 여러개의 Cell을 가진다.
			 */
			...
			if(row != null) {
				...
				for(int cellIndex = 0; cellIndex < numOfCells; cellIndex++) {
					...
					/*
					 * Row 에서 CellIndex 에 해당하는 Cell을 가져온다.
					 */
					...
					/*
					 * 맵 객체에 Cell의 이름을 키(Key)로 데이터를 담는다.
					 */
